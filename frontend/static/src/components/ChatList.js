...
class ChatList extends Component {
  ...
  componentDidMount() {
    this.displayMessages = setInterval(this.fetchMessages, 1000);
  }
  ...
  fetchMessages() {
    fetch('/api/v1/chats/').then(response => response.json()).then(data => this.setState({messages: data}));
  }
  ...
  deleteMessage(id) {
    ...
    fetch(`/api/v1/chats/${id}/`, options).then(response => {
      if (!response.ok) {
  ...
  addMessage(event) {
    ...
    fetch('/api/v1/chats/', options).then(response => response.json()).then(data => {
      const messages = [...this.state.messages];
      messages.push(data);
      this.setState({messages});
      this.setState({text: ''});
